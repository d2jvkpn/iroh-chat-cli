...
fn main() {
    let run = |args: Vec<&str>| process::Command::new(args[0]).args(&args[1..]).output();
    let output_utf8 = |v: process::Output| String::from_utf8_lossy(&v.stdout).trim().to_string();

    let rustc_version = run(vec!["rustc", "--version"])
        .ok()
        .map(output_utf8)
